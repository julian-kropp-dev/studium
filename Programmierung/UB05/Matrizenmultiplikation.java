...
public class Matrizenmultiplikation {
    public static void main(String[] args) {
        ...
        if (!test(mata, matb)) { // Teste ob Multipliaktion erlaubt ist
            System.out.println("MatError Spaltenzahl von Mat A ist nicht gleich Zeilenzahl von Mat B!");
        } else {
            double[][] result = new double[matarow][matbcol]; // Erebnis Matrix C mit den Zeilen von A und den Spalten
                                                              // von B
    ...
    // Geg: Zeile Mat A (z)
    // Spalte Mat B (s)
    // Errechnet das Ergebnis für Cz, indem die Produkte der entsprechenden Einträge
    // der geg. Zeile von Mat A und der geg. Spalte von Mat B aufsummiert werden
